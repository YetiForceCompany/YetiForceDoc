...
- **Baza danych**
  ...
  - **MySQL `8.0.13`** - lub nowsza wersja. Zalecamy użycie najnowszej stabilnej wersji oprogramowania.
...
server {
  ...
  root /path/to/yetiforce/public_html;
...
#### SabreDAV i Roundcube

Jeżeli korzystasz z **SabreDAV** lub **Roundcube**, dodaj poniższe reguły do bloku `server`.
...
Część serwerów umożliwia zmianę konfiguracji za pomocą pliku `.htaccess`. Poniżej przedstawiamy kilka przykładów.
